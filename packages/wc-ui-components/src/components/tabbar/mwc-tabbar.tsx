...
export class MWCTabbar {
  ...
  componentDidLoad() {
    if (this.scroller) {
      this.mdcTabBarComponent = MDCTabBarScroller.attachTo(
        this.mdcTabBarScroller
      );
    } else {
  ...
  renderText(tab) {
    if (this.iconTabBar) {
      return <span class="mdc-tab__icon-text">{tab.label}</span>;
    }
    return tab.label;
  }
  renderIcon(tab) {
    if (this.iconTabBar) {
      return (
        <i class="material-icons mdc-tab__icon" aria-hidden="true">
          {tab.icon}
        </i>
      );
    }
  ...
  renderNavigation() {
    return (
      <nav
        class={this.getTabBarClassName()}
        ref={(mdcTabBar) => {
          this.mdcTabBar = mdcTabBar;
        }}
      >
