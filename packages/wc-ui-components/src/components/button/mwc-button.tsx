...
export class MWCButton {
  ...
  componentWillLoad() {
    ...
    this.btnEl.addEventListener('click', (evt) => {
      this.onClick.emit({ ...evt, alex: 'kuku' });
    });
  }
  ...
  componentDidLoad() {
    if (this.ripple) {
      const rippleColor =
        this.raised && (this.color === 'primary' || this.color === 'secondary')
          ? rippleColors.white
  ...
  renderIcon() {
    if (this.icon) {
      return <i class="material-icons mwc-icon">{this.icon}</i>;
    }
    return null;
  }
  renderButton() {
