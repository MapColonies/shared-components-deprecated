...
export class MWCCheckbox {
  ...
  @Event() changeHandler: EventEmitter;
  inputComp: any;
  checkRipple: any;
  innerStyles: any;
  checkboxDiv: any;
  ...
  componentWillLoad() {
    if (this.checkcolor) {
      this.checkBoxEl.style.setProperty(
        '--mdc-theme-secondary',
        `#${this.checkcolor}`
      );
    }
    ...
    return (
      ...
      >
        <input
          ...
          onChange={(evt) => this.changeHandler.emit(evt)}
  ...
  renderCheckboxWithLabel() {
    return (
      <div
        ...
        ref={(formFieldDiv) => {
          this.formFieldDiv = formFieldDiv;
        }}
      >
