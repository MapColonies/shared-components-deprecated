const touchMyRipple = () => {
  ...
  function ripple(els, rippleColor, eventListener) {
    ...
    currentBtn.addEventListener(eventListener, function (e) {
      ...
      if (
        eventListener.match(/touch/) &&
        eventListener.match(/touch/)[0].length > 0
      ) {
      ...
      if (
        window.getComputedStyle(this).position !== 'fixed' &&
        window.getComputedStyle(this).position !== 'absolute'
      ) {
  ...
  const tmripple = {
    ...
    attachToSelectors(data) {
      try {
        ...
        const eventListener =
          data.eventListener || defaultSettings.eventListener;
