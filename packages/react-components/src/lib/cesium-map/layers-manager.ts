...
class LayerManager {
  ...
  public removeLayer(layerId: string): void {
    const layer = this.mapViewer.imageryLayers._layers.find((layer) => {
      return layer.meta ? layer.meta.id === layerId : false;
    });
  ...
  public removeBaseMapLayers(): void {
    const layerToDelete = this.mapViewer.imageryLayers._layers.filter(layer => {
      return layer.meta?.parenBasetMapId ? true: false;
    });
